...
def mse(x, theta, y):
    ...
    Parameters:
    x (2D list or numpy array): samples (x_i)
    theta (list or numpy array) 1D: The weights for the function
    y (list or numpy array) 1D: Truth values
    Returns:
...
def gradient_descent(x, y, theta, itr, alpha):
    ...
    Parameters:
    y (list or numpy array) 1D: Truth values
    x (list or numpy array) 2D: dataset.
    alpha (float): learning rate
    Returns:
    prev_thetas: List of gradient descent temporary thetas
    prev_mse: value (float) of minimal MSE
    """
    # Append ones feature for matrix operations
    x = np.c_[np.ones(x.shape[0]), x] 
    prev_mse = [] # MSE results with different weights 
    prev_thetas = [theta] # Model coefs
    for i in range(itr):
        prev_mse.append(mse(x, theta, y)) # MSE value with current weights
        theta = theta - (alpha * mse_gradient(x, theta, y)) # Theta change
